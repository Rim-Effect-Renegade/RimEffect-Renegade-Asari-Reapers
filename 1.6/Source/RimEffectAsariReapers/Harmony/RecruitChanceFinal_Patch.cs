namespace RimEffectAR
{
    ...
    public static class RecruitInteractionWorker_Patch
    {
        ...
        {
            if (initiator.health?.hediffSet.HasHediff(RimEffectARDefOf.RE_Biotic_CharmHediff) ?? false)
        ...
        {
            Hediff def = initiator.health.hediffSet.GetFirstHediffOfDef(RimEffectARDefOf.RE_Biotic_CharmHediff);
            if (def != null)
            {
                initiator.health.hediffSet.hediffs.Remove(def);
                recipient.health?.AddHediff(RimEffectARDefOf.RE_Biotic_CharmedHediff);
                recipient.needs.mood?.thoughts.memories.TryGainMemory(RimEffectARDefOf.RE_CharmThought, initiator);
            }
        }
    }
}
