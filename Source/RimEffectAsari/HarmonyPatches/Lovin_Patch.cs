namespace RimEffectAsari
{
    ...
    public static class LovinJobDriver_Patch
    {
        ...
        {
            if (pawn.genes.Xenotype == REA_DefOf.RE_Asari || partner.genes.Xenotype == REA_DefOf.RE_Asari)
            {
    ...
    public static class LovinJobDriverFleck_Patch
    {
        ...
        public static IntVec3 GetBioticPos(Pawn pawn, Pawn partner) => 
            pawn.genes.Xenotype == REA_DefOf.RE_Asari ? pawn.Position + ((partner.Position.ToVector3() - pawn.Position.ToVector3())/2).ToIntVec3() : pawn.Position;

        public static FleckDef GetLovinFleck(Pawn pawn)
        {
            if (pawn.genes.Xenotype == REA_DefOf.RE_Asari) 
    ...
    public static class JobDriverReportString_Patch
    {
        ...
        public static void Prefix(JobDriver __instance, ref string str)
        {
            if(__instance.job.def == JobDefOf.Lovin)
                if (__instance.pawn.genes.Xenotype == REA_DefOf.RE_Asari || ((Pawn) __instance.job.GetTarget(TargetIndex.A)).genes.Xenotype == REA_DefOf.RE_Asari)
    ...
    public static class SecondaryLovinChanceFactor_Patch
    {
        ...
        {
            if ((___pawn.genes.Xenotype == REA_DefOf.RE_Asari || otherPawn.genes.Xenotype == REA_DefOf.RE_Asari) && ___pawn.def != otherPawn.def)
                __result *= 2f;
