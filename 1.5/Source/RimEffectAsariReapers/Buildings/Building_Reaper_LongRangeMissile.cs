...
namespace RimEffectAR
{
    ...
    public class Building_Reaper_LongRangeMissile : Building_Reaper, IAttackTargetSearcher
    {
        ...
        public void Fire()
        {
            var missile = (LongRangeMissile) SkyfallerMaker.SpawnSkyfaller(RimEffectARDefOf.RER_ReaperLongRangeMissile_Leaving, Position, Map);
            missile.Init(this);
            RimEffectARDefOf.RE_Launch_ReaperLongRangeMissile.PlayOneShot(SoundInfo.InMap(missile, MaintenanceType.PerTick));
        }
        ...
        public void Incoming()
        {
            ...
            var missile = (LongRangeMissile) SkyfallerMaker.SpawnSkyfaller(RimEffectARDefOf.RER_ReaperLongRangeMissile_Arriving, target.Position, Map);
            missile.Init(this);
            missile.angle = 0; //new Vector3(Map.Size.ToVector3().x, target.DrawPos.y, DrawPos.z).AngleToFlat(target.DrawPos);
            RimEffectARDefOf.RE_Incoming_ReaperLongRangeMissile.PlayOneShot(SoundInfo.InMap(missile, MaintenanceType.PerTick));
        }
    ...
    public class LongRangeMissile : Skyfaller
    {
        ...
        public override void Tick()
        {
            ...
            FleckCreationData data = FleckMaker.GetDataStatic(drawPos, this.Map, RimEffectARDefOf.RER_ReaperGlow, Rand.Range(4f, 6f) * 2);
