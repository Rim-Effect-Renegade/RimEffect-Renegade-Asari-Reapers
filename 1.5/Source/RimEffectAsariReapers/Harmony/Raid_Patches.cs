...
namespace RimEffectAR.Harmony
{
    internal class Raid_Patches
    {
        ...
        private static class FactionForCombatGroup_Patch
        {
            ...
            {
                if (f.def != RimEffectARDefOf.RE_Reapers || ReaperUtils.ReaperPresence() > 1) return true;
                __result = 0f;
                return false;
            }
        }
        ...
        private static class RaidEnemyResolveFaction_Patch
        {
            ...
            public static void Prefix(ref IncidentParms parms)
            {
                if (parms.faction is null)
                    return;
                if (parms.faction.def == RimEffectARDefOf.RE_Reapers && ReaperUtils.ReaperPresence() <= 0)
                    parms.faction = null;
            }
            ...
            {
                if (parms.faction?.def == RimEffectARDefOf.RE_Reapers)
