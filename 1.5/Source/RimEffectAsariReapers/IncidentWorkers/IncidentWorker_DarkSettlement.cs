...
namespace RimEffectAR
{
    ...
    public class IncidentWorker_DarkSettlement : IncidentWorker
    {
        ...
        {
            IEnumerable<IGrouping<Faction, Settlement>> validBases = Find.WorldObjects.Settlements.GroupBy(s => s.Faction).Where(g => g.Key.def != RimEffectARDefOf.RE_Reapers && !g.Key.IsPlayer);
